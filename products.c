...
void print_products(struct dynarray* products) {
  ...
  for(int i = 0; i < length; i++) {
    struct product* product = (struct product*)(dynarray_get(products, i));
    printf("Product %d: Name: %s, Inventory: %d, Price: %f\n", i, product->name, product->inventory, product->price);
  }
}
...
void sort_by_inventory(struct dynarray* products) {
  ...
  for(int i = 0; i < n - 1; i++)
    for(int j = 0; j < n - i - 1; j++) {
      struct product* product = (struct product*)(dynarray_get(products, j));
      struct product* next_product = (struct product*)(dynarray_get(products, j+1));
      if(product->inventory > next_product->inventory)
        swap(&product, &next_product);
    }
}